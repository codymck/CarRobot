...
#define SERVO_PIN 15
#define SERVO_LEFT 440 //ultrasonic sensor facing right 585
#define SERVO_CENTER 310 //ultrasonic sensor facing front 350
#define SERVO_RIGHT 170 //ultrasonic sensor facing left 135
#define TRIGGER 28
#define ECHO 29
#define OBSTACLE 40
#define FOLLOW_RANGE 30
...
float distance()
{
  ...
  //printf("elapsed: %f\n", elapsed);
  dis = ((elapsed * 34300) / 2) / 1000; // in centimeters
  printf("distance: %f\n", dis);
  return dis;
}
...
// handles the control c end event
...
/* die */
// ends process sends error to screen
void die(char *s)
{ perror(s); exit(1); }
...
void udp_handler(char p){
  while(1)
  {
    ...
    switch(cur_status)
    {
      ...
      case 'L': left(fd, speed, speed); delay(150); stop(fd); cur_status = p; break;
      case 'R': right(fd, speed, speed); delay(150); stop(fd); cur_status = p; break;
